...
function App() {
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      ...
      alert('Please install MetaMask to use Noah Protocol.');
    }
  };
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  {chainId !== NOAH_CHAIN_ID && (
                    <button onClick={switchNetwork} style={{
                      ...
                      <AlertCircle size={14} /> Switch to Sepolia
                    </button>
                  )}
        ...
        <nav style={{
          ...
          <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer' }} onClick={() => setView('landing')}>
              ...
              <span style={{ fontSize: '1.125rem', fontWeight: 800, letterSpacing: '-0.02em' }}>NOAH</span>
            </div>
